...
module.exports.fft = (file) => {
  ...
  //take data from object and place into arrays
  let idx = 1;
  ...
  //
  //grab all fft from 25,000 to EOF - 4096
  for (let start = 25000; start < signal1.length - epochSize; start = start + epochSize) {
    returnObj = {};
    f.realTransform(out, signal1.slice(start, start + epochSize));

    let magnitude, real, imag;
    console.log(Date.now() - startTime + ' ms since start | ' + 'current start point: ' + start + 'signal 1 length: ' + signal1.length);
  ...
  //return 'stuff';
  console.log('epochs processed: ', FFTArray.length, FFTArray[0]['0.146'], FFTArray[FFTArray.length - 2]['0.146'], FFTArray[FFTArray.length - 1]);
  //
  return returnObj;
