...
//function to parse raw edf file
...
//perform FFT on all epochs in ASCII formatted EEG file
module.exports.fft = (file) => {
  //console.log('Performing FFT on all epochs');
  if (file) {
    my_edf = fs.readFileSync(file);
  } else {
    my_edf = fs.readFileSync("./2017-10-07 TEST_data.txt");
...
//return an array of objects based on the FFT array created by fft() whose keys are within the given frequencies
...
let getMaxPoint = (object) => {
  //assume string of three significant figure floating point number for key and value pairs
  try{
...
//wrapper function to grab array of max points for array of FFT objects
module.exports.getMaxPointArray = (inputFFTArray) => {
  return inputFFTArray.map(getMaxPoint);
};

//returns area under curve for individual object of frequency, mangnitude pairs
let getIntegral = (object) => {
  try{
    let keys = Object.keys(object);
    let halfEndValues = (parseFloat(object[keys[0]]) + parseFloat(object[keys[keys.length - 1]])) / 2
    let timeStep = parseFloat(keys[1]) - parseFloat(keys[0]);

    //add all trapezoid areas
    let result = keys.reduce((sum, curKey) => {
      let parsedKey = parseFloat(curKey);
      let parsedVal = parseFloat(object[curKey]);
      return sum + parsedVal;
    }, 0);

    //subtract half of ends since they are only used in one area calculation, while the rest of the
    //magnitude values are used in two trapezoidal area calculations
    result = (result - halfEndValues) * timeStep;
    return result.toFixed(3);
  }
  catch (e) {
    console.error('error in getIntegral', ex, ex.message);
  }
};

// wrapper function to return array of integrals for curves given
module.exports.getIntegralArray = (inputFFTArray) => {
  return inputFFTArray.map(getIntegral);
};